...
WPAC.LoadComments = function (url, options) {
  ...
  var request = jQuery.ajax({
    ...
    success: function success(data) {
      try {
        if (!WPAC._ReplaceComments(data, resolvedLoadUrl, true, formData, formFocus, options.selectorCommentsContainer, options.selectorCommentForm, options.selectorRespondContainer, options.beforeSelectElements, options.beforeUpdateComments, options.afterUpdateComments)) {
          return;
        }
        if (options.updateUrl) {
