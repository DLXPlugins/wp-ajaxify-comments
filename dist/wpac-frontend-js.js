...
/**
 * Resolve any URL string to an absolute href using one code path (relative URLs, empty header values, etc.).
 *
 * @param {string|undefined|null} url    Candidate URL.
 * @param {string}                [base] Base URL (e.g. current page); defaults to window.location.href. Resolved internally as baseUrlLocation.
 * @return {{ href: string, valid: boolean }} Always an object (never null/false). `valid` is false when `href` may not parse with `new URL()`.
 */
WPAC._resolveUrl = function (url, base) {
  var baseUrlLocation = typeof base === 'string' && base.trim() !== '' ? base : window.location.href;
  var fallbackToBase = function fallbackToBase() {
    try {
      return {
        href: new URL(baseUrlLocation).href,
        valid: true
      };
    } catch (err) {
      return {
        href: String(baseUrlLocation),
        valid: false
      };
    }
  };
  ...
  var candidateUrl = String(url).trim();
  if (candidateUrl === '') {
    return fallbackToBase();
  }
  try {
    return {
      href: new URL(candidateUrl, baseUrlLocation).href,
      valid: true
    };
  } catch (e) {
    WPAC._Debug('info', "Could not resolve URL '%s' against base URL; using base URL instead.", candidateUrl);
    return fallbackToBase();
  }
};

/**
 * Parse {@link WPAC._resolveUrl} output into a URL instance. Never relies on `valid` alone: always try/catch and fall back to the current page.
 *
 * @param {{ href: string, valid: boolean }|null|undefined} resolved Result from WPAC._resolveUrl.
 * @return {URL} Always returns a URL; uses window.location on failure.
 */
WPAC._urlFromResolved = function (resolved) {
  try {
    if (resolved && typeof resolved.href === 'string' && resolved.href !== '') {
      return new URL(resolved.href);
    }
  } catch (e) {
    WPAC._Debug('info', "Resolved href was invalid ('%s'); using current page URL.", resolved && resolved.href);
  }
  return new URL(window.location.href);
};
WPAC._AddQueryParamStringToUrl = function (url, param, value) {
  var urlObject = WPAC._urlFromResolved(WPAC._resolveUrl(url, window.location.href));
...
WPAC._LoadFallbackUrl = function (fallbackUrl) {
  var callerContext = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  jQuery.unblockUI();
  WPAC._ShowMessage(WPAC._Options.textReloadPage, 'loading');
  var url;
  try {
    var resolved = WPAC._resolveUrl(fallbackUrl, window.location.href);
    url = WPAC._AddQueryParamStringToUrl(resolved.href, 'WPACRandom', new Date().getTime());
  } catch (e) {
    WPAC._Debug('error', 'Fallback reload URL invalid, retrying with current location: %o', e);
    try {
      url = WPAC._AddQueryParamStringToUrl(window.location.href, 'WPACRandom', new Date().getTime());
    } catch (e2) {
      window.location.reload();
      return;
    }
  }
  WPAC._Debug('info', "Something went wrong. Reloading page (caller: '%s', URL: '%s')...", callerContext || '(unspecified)', url);
...
WPAC._findInExtractedBody = function (root, rawSelector, prefixedSelector) {
  var raw = rawSelector !== null && typeof rawSelector === 'string' && rawSelector.trim() !== '' ? rawSelector : prefixedSelector;
  if (raw === prefixedSelector) {
    return root.find(prefixedSelector);
  }
  var $match = root.find(raw);
  if ($match.length) {
    return $match;
  }
  $match = root.find(prefixedSelector);
  if ($match.length) {
    WPAC._Debug('info', 'Fetched markup: raw selector matched nothing; using prefixed selector as fallback.');
  }
  return $match;
};
WPAC._ReplaceComments = function (data, commentUrl, useFallbackUrl, formData, formFocus, selectorCommentsContainer, selectorCommentForm, selectorRespondContainer, beforeSelectElements, beforeUpdateComments, afterUpdateComments, selectorCommentsContainerRaw, selectorCommentFormRaw, selectorRespondContainerRaw) {
  ...
  if (!oldCommentsContainer.length) {
    ...
    WPAC._LoadFallbackUrl(fallbackUrl, 'WPAC._ReplaceComments: comment container missing on current page');
    return false;
  }
  ...
  if (extractedBody === false) {
    ...
    WPAC._LoadFallbackUrl(fallbackUrl, 'WPAC._ReplaceComments: unsupported server response (extract body)');
    return false;
  }
  ...
  var newCommentsContainer = WPAC._findInExtractedBody(extractedBody, selectorCommentsContainerRaw, selectorCommentsContainer);
  if (!newCommentsContainer.length) {
    ...
    WPAC._LoadFallbackUrl(fallbackUrl, 'WPAC._ReplaceComments: comment container missing on requested page');
    return false;
  }
  if (newCommentsContainer.length > 1) {
    ...
    var respondContainer = WPAC._findInExtractedBody(newCommentsContainer, selectorRespondContainerRaw, selectorRespondContainer);
  ...
  if (WPAC._Options.commentsEnabled) {
    ...
    if (form.length) {
      ...
      if (!form.parents(selectorCommentsContainer).length) {
        ...
        var newCommentForm = WPAC._findInExtractedBody(extractedBody, selectorCommentFormRaw, selectorCommentForm);
        if (newCommentForm.length == 0) {
          WPAC._Debug('error', "Comment form on requested page not found (selector: '%s')", selectorCommentForm);
          WPAC._LoadFallbackUrl(fallbackUrl, 'WPAC._ReplaceComments: comment form missing on requested page');
          return false;
        }
      ...
      if (!wpTempFormDiv.length) {
        ...
        WPAC._LoadFallbackUrl(fallbackUrl, 'WPAC._ReplaceComments: #wp-temp-form-div missing');
        return false;
      }
      var newRespondContainer = WPAC._findInExtractedBody(extractedBody, selectorRespondContainerRaw, selectorRespondContainer);
      if (!newRespondContainer.length) {
        ...
        WPAC._LoadFallbackUrl(fallbackUrl, 'WPAC._ReplaceComments: respond container missing on requested page');
        return false;
      }
...
WPAC.AttachForm = function (options) {
  ...
  // Delegated handlers: jQuery( document ).on( eventName, selector, fn ).
  // Events bubble to document; jQuery runs fn when some node on the path from event.target to document matches selector (this === that node).
  ...
  if (maybeSelectorCommentPagingEl.length > 0) {
    ...
    var navSelector = '#comments nav a';
    var navEl = jQuery(navSelector);
    if (navEl.length > 0) {
  ...
  var linkClickHandler = function linkClickHandler(event) {
    ...
    if (href) {
      try {
        anchor = new URL(href, window.location.href).hash;
      } catch (err) {
        WPAC._Debug('info', "Skipping comment link with invalid href: '%s'", href);
        return;
      }
    }
    ...
    var request = jQuery.ajax({
      ...
      success: function success(data) {
        ...
        if (request.getResponseHeader('X-WPAC-ERROR')) {
          WPAC._Debug('info', 'Found error state X-WPAC-ERROR header.');
        ...
        if (WPAC._Options.useCurrentPageForCommentRefresh) {
          commentUrl = window.location.href;
          WPAC._Debug('info', 'Using current page URL for comment refresh (setting enabled).');
        } else {
          commentUrl = WPAC._urlFromResolved(WPAC._resolveUrl(commentUrl, window.location.href)).href;
        }
        ...
        // Replace comments while the posting overlay is still visible; then unblock and show success.
        if (!WPAC._ReplaceComments(data, commentUrl, false, {}, '', options.selectorCommentsContainer, options.selectorCommentForm, options.selectorRespondContainer, options.beforeSelectElements, options.beforeUpdateComments, options.afterUpdateComments, options.selectorCommentsContainerRaw, options.selectorCommentFormRaw, options.selectorRespondContainerRaw)) {
          return;
        }
        jQuery.unblockUI();
        ...
        if (commentUrl) {
          ...
          if (WPAC._hasPostContainer() && options.updateUrl && !WPAC._Options.useCurrentPageForCommentRefresh) {
            WPAC._Debug('info', 'Single-page comment forms detected, but single-page refresh if disabled, so reloading the page.');
            window.location.reload();
            return;
          }
      ...
      error: function error(jqXhr, textStatus, errorThrown) {
        ...
        if (jqXhr.status === 0 && jqXhr.responseText === '') {
          ...
          WPAC._LoadFallbackUrl(WPAC._AddQueryParamStringToUrl(window.location.href, 'WPACFallback', '1'), 'WPAC.AttachForm: comment POST follow-up GET failed (status 0)');
          return;
        }
  ...
  // No separate "scope" argument: the filter is one CSS selector. Post scoping is part of that string.
  // Init calls AttachForm( { selectorCommentForm: _scopeSelectorToPostContainer( id, option ), ... } ) per post container, or AttachForm() using the raw option.
...
WPAC.Init = function () {
  ...
  if (WPAC._hasPostContainer()) {
    ...
    if (attachedCount === 0) {
      WPAC._Debug('error', "Post container selector matched no usable elements (need matching nodes with an id). Falling back to WPAC.AttachForm() without post scoping. Selector: '%s'", WPAC._Options.selectorPostContainer);
      WPAC.AttachForm();
    }
  } else {
    WPAC.AttachForm();
...
WPAC.LoadComments = function (url, options) {
  ...
  var urlObject = WPAC._urlFromResolved(WPAC._resolveUrl(url, window.location.href));
  var resolvedLoadUrl = urlObject.href;
  ...
  if (queryParams.has('ajaxifyLazyLoadEnable')) {
    ...
    // Add to URL.
    resolvedLoadUrl = WPAC._AddQueryParamStringToUrl(resolvedLoadUrl, 'ajaxifyLazyLoadEnable', ajaxifyLazyLoadEnable);
    resolvedLoadUrl = WPAC._AddQueryParamStringToUrl(resolvedLoadUrl, 'nonce', nonce);
    resolvedLoadUrl = WPAC._AddQueryParamStringToUrl(resolvedLoadUrl, 'post_id', postId);
  }
  if (options.disableCache) {
    resolvedLoadUrl = WPAC._AddQueryParamStringToUrl(resolvedLoadUrl, 'WPACRandom', new Date().getTime());
  }
  jQuery.ajax({
    url: resolvedLoadUrl,
    type: 'GET',
    ...
    success: function success(data) {
      try {
        if (!WPAC._ReplaceComments(data, resolvedLoadUrl, true, formData, formFocus, options.selectorCommentsContainer, options.selectorCommentForm, options.selectorRespondContainer, options.beforeSelectElements, options.beforeUpdateComments, options.afterUpdateComments, options.selectorCommentsContainerRaw, options.selectorCommentFormRaw, options.selectorRespondContainerRaw)) {
          return;
        }
        if (options.updateUrl) {
          WPAC._UpdateUrl(resolvedLoadUrl);
        }

        // Scroll to anchor
        var waitForScrollToAnchor = false;
        if (options.scrollToAnchor) {
          var _anchor2 = resolvedLoadUrl.indexOf('#') >= 0 ? resolvedLoadUrl.substr(resolvedLoadUrl.indexOf('#')) : null;
          if (_anchor2) {
    ...
    error: function error() {
      WPAC._LoadFallbackUrl(WPAC._AddQueryParamStringToUrl(window.location.href, 'WPACFallback', '1'), 'WPAC.LoadComments: GET comments failed');
    }
  });
  return true;
};
jQuery(function () {
  ...
  if (true === WPAC._Options.lazyLoadEnabled) {
    if (!initSuccesful) {
      WPAC._LoadFallbackUrl(WPAC._AddQueryParamStringToUrl(window.location.href, 'WPACFallback', '1'), 'jQuery ready: lazy load enabled but WPAC.Init failed');
      return;
    }
...
/* harmony import */ var _wp_ajaxify_comments__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_wp_ajaxify_comments__WEBPACK_IMPORTED_MODULE_4__);
