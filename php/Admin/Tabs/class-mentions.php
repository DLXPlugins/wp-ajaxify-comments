...
/**
 * Output the mentions tab and content.
 */
class Mentions {
	/**
	 * Constructor.
	...
	public function __construct() {
		...
		add_filter( 'comment_edit_pro_output_mentions', array( $this, 'output_main_content' ), 1, 3 );
	}
	...
	public function add_main_tab( $tabs ) {
		$tabs[] = array(
			'get'    => 'mentions',
			'action' => 'comment_edit_pro_output_mentions',
			'url'    => Functions::get_settings_url( 'mentions' ),
			'label'  => _x( 'Mentions (Beta)', 'Tab label as Main', 'ultimate-auto-updates' ),
			'icon'   => 'at',
		);
		return $tabs;
	}
	...
	public function add_main_main_sub_tab( $tabs, $current_tab, $sub_tab ) {
		if ( ( ! empty( $current_tab ) || ! empty( $sub_tab ) ) && 'mentions' !== $current_tab ) {
			return $tabs;
		}
		return $tabs;
	}
	...
	public function output_main_content( $tab, $sub_tab = '' ) {
		if ( 'mentions' === $tab ) {
			if ( empty( $sub_tab ) || 'mentions' === $sub_tab ) {
				?>
				<div class="sce-admin-panel-area">
					<div class="sce-panel-row">
						<div id="sce-tab-mentions"><?php echo wp_kses( Functions::get_loading_svg(), Functions::get_kses_allowed_html( true ) ); ?></div>	
					</div>
				</div>
				<?php
			}
