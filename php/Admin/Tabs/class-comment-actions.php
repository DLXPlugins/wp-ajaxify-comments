<?php
/**
 * Output the comment actions tab.
 *
...
class Comment_Actions {
	...
	public function __construct() {
		...
		add_filter( 'comment_edit_pro_output_comment-actions', array( $this, 'output_main_content' ), 1, 3 );
	}
	...
	public function add_main_tab( $tabs ) {
		$tabs[] = array(
			'get'    => 'comment-actions',
			'action' => 'comment_edit_pro_output_comment-actions',
			'url'    => Functions::get_settings_url( 'comment-actions' ),
			'label'  => _x( 'Comment Actions', 'Tab label as Main', 'ultimate-auto-updates' ),
			'icon'   => 'person-running',
		);
		return $tabs;
	}
	...
	public function output_main_content( $tab, $sub_tab = '' ) {
		if ( 'comment-actions' === $tab ) {
			if ( empty( $sub_tab ) || 'comment-actions' === $sub_tab ) {
				?>
